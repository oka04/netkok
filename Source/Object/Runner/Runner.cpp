...
void Runner::UpdateFromNetwork(const NetPlayerState& state, DirectionalLight& light, float deltaTime)
{
	...
	// ★★★ 重要: サーバーの状態を無条件に信頼して反映（ホストの判定が正しい）★★★
	...
	if (wasFrozen != newFrozen)
	{
		...
		else
		{
			...
			NET_LOG_F("[Runner::UpdateFromNetwork] ID=%u ★★★完全解凍★★★",
				m_clientId);
		}
	...
	{
		// ★★★ 凍結継続中 - サーバーの解凍進行を常に反映 ★★★
		if (abs(netAmount - oldAmount) > 0.001f)
		{
			m_frozenAmount = netAmount;
			...
			if (m_frozenAmount >= 1.0f)
			{
				...
				NET_LOG_F("[Runner::UpdateFromNetwork] ID=%u ★★★1.0到達により完全解凍★★★",
					m_clientId);
			}
		}
	}
	...
	if (state.meltTargetId != m_targetMeltPlayer)
	{
		...
		{
			NET_LOG_F("[Runner::UpdateFromNetwork] ID=%u meltTarget: %u -> %u",
