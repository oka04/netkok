		...
		if (abs(netAmount - m_frozenAmount) > 0.001f)
		{
			...
			if (m_frozenAmount > oldAmount)
			{
				NET_LOG_F("[Runner::UpdateFromNetwork] ID=%u 解凍進行: %.3f -> %.3f",
					m_clientId, oldAmount, m_frozenAmount);
			}
			else
			{
				NET_LOG_F("[Runner::UpdateFromNetwork] ID=%u 解凍情報更新（増加なし）: %.3f",
					m_clientId, m_frozenAmount);
			}
...
{
	// ★★★ 修正: deltaTimeを乗算しない（秒速のまま保持）★★★
	if (m_bFatigued)
	{
		m_speed = f_fatigueSpeed; // 3.0 m/s
	}
	else if (m_keyFlag & CROUCH_KEY)
	{
		m_speed = f_crouchSpeed; // 1.5 m/s
	}
	else if (m_keyFlag & DASH_KEY)
	{
		m_speed = f_dashSpeed; // 6.0 m/s
	}
	else
	{
		m_speed = f_walkSpeed; // 5.0 m/s
	}
	...
	{
		NET_LOG_F("[Runner::ChangeSpeed] ID=%u IsLocal=%s Speed=%.3f (秒速) Dash=%s Crouch=%s Fatigue=%s",
			m_clientId,
			m_bIsLocal ? "Yes" : "No",
			m_speed,
